#+TITLE: Impermanence
  ...
  - At least one of the modules in this repository, which take care of
    symlinking files and directories between the persistent storage
...
***** ~"/persistent"~
      ...
      - ~hideMounts~ used to hide the bind mounts from showing up as
        mounted drives in the file manager. It has no effect now that
        directories are symlinked rather than bind mounted (symlinks
        don't show up as mounted drives), and is kept only for
        backwards compatibility.

      - ~allowTrash~ used to allow apps that use certain GTK-based
        technologies such as GIO to put items in the trash. It has no
        effect now that directories are symlinked rather than bind
        mounted (a symlinked directory shares a filesystem with its
        trash location, so trashing already works), and is kept only
        for backwards compatibility.

****** ~directories~

       All directories you want to symlink to persistent storage. A
       ...
       - ~directory~, the path to the directory you want to symlink to
...
****** ~files~
       ...
       All files you want to link to persistent storage. A
       ...
       - ~file~, the path to the file you want to symlink to
         persistent storage. Only setting this option is equivalent to
         the string representation.
       ...
       - ~method~, the method used to link the file to persistent
         storage. ~"symlink"~, the default, points a symlink at the file
         in persistent storage. This avoids the cross-device copies a
         bind mount forces when a program renames a file into place.
         ~"auto"~ uses a bind mount when the file exists in persistent
         storage and a symlink otherwise; only use it if ~"symlink"~ is
         giving you issues. Note that ~/etc/machine-id~ is always bind
         mounted regardless of this setting, as symlinking it breaks
...
****** ~directories~
       ...
       All directories you want to symlink to persistent
       storage. See [[#directories][the NixOS module directories section]] for more
       details.

****** ~files~

       All files you want to symlink to persistent storage. See
